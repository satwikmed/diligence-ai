...
export async function getAnalysisStatus(documentId: string): Promise<AnalysisStatus> {
  const res = await fetch(`${API_URL}/api/analysis/${documentId}/status`);
  if (!res.ok) throw new Error('Failed to fetch status');
  return res.json();
}

export async function getAnalysis(documentId: string): Promise<AnalysisReport> {
  const res = await fetch(`${API_URL}/api/analysis/${documentId}`);
  if (!res.ok) throw new Error('Failed to fetch analysis');
  return res.json();
}

export async function askQuestion(documentId: string, question: string) {
  const res = await fetch(`${API_URL}/api/analysis/${documentId}/ask`, {
    method: 'POST',
...
export async function deleteAnalysis(documentId: string) {
  const res = await fetch(`${API_URL}/api/history/${documentId}`, { method: 'DELETE' });
  if (!res.ok) throw new Error('Delete failed');
  return res.json();
}

export async function compareCompanies(doc1: string, doc2: string) {
  const res = await fetch(`${API_URL}/api/compare?doc1=${doc1}&doc2=${doc2}`);
  if (!res.ok) throw new Error('Compare failed');
  return res.json();
}

export async function getAgentLogs(documentId: string) {
  const res = await fetch(`${API_URL}/api/agent-logs/${documentId}`);
  if (!res.ok) throw new Error('Failed to fetch logs');
  return res.json();
}

export async function getSuggestedQuestions(documentId: string) {
  const res = await fetch(`${API_URL}/api/analysis/${documentId}/suggested-questions`);
